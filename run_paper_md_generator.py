...
def generate_markdown_from_pdfs(pdf_to_md_generator, papers_pdf_folder, pdf_mds_folder):
    ...
    for pdf_file in os.listdir(papers_pdf_folder):
        if pdf_file.endswith('.pdf'):
            ...
            try:
                output_file_vision = os.path.join(pdf_mds_folder, f"{os.path.splitext(pdf_file)[0]}_vision_desc_{timestamp}.md")
                ...
                try:
                    output_file_text = os.path.join(pdf_mds_folder, f"{os.path.splitext(pdf_file)[0]}_text_desc_{timestamp}.md")
